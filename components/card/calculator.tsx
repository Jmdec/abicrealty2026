...
// Table Headers (left aligned for consistency with data)
doc.setFont("helvetica", "normal");
doc.setFontSize(12);
doc.text("LOAN DETAILS", 20 + 10, 58); // left aligned inside first column (near 30)
doc.text("VALUE", 120 + 10, 58);       // left aligned inside second column (near 130)
...
// Table Rows Data
const rows = [
  ["Years", `${selectedYears || 0} Year(s)`],
  ["Months", `${selectedMonths || 0} Month(s)`],
  [
    "Loan Amount",
...
// Table Content
let yPosition = 72;
const rowHeight = 12; // spacing between rows

rows.forEach((row) => {
  doc.text(row[0], 30, yPosition);  // left aligned in first column (near 30)
  doc.text(row[1], 130, yPosition); // left aligned in second column (near 130)
  doc.line(20, yPosition + 4, 190, yPosition + 4); // line below the text
  yPosition += rowHeight;
});

// Draw Table Borders
const startY = 52;
const endY = yPosition - (rowHeight - 4);
...
// Footer Content
const footerText1 = "ABIC Realty & Consultancy Corporation";
const footerText2 = "Unit 202, Campos Rueda Building, 101 Urban Ave, Makati, 1206 Metro Manila";
const footerText3 = "Phone Number: (+63)9175-4809-99 | Email: abicrealty@gmail.com";
      ...
      <Table aria-label="Loan Calculation Results">
        ...
        <TableBody>
          ...
          <TableRow key="4">
            ...
            <TableCell>{interestRate || 0}%</TableCell>
          </TableRow>
          <TableRow key="5">
